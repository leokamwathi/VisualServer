...
public class HeartSpread : Particles2D
{
	...
	{
		if (Visible == false && cooldown ==0 && Input.IsActionPressed("HEART") && GetParent().GetChildCount() < 1000)
		{
			Particles2D heart = (Particles2D)Duplicate();
			Vector2 ScreenCenter = new Vector2(GetParent().GetViewport().Size / 2);
			heart.Position = ScreenCenter;
			heart.Lifetime = lifeTime/2;
			((ParticlesMaterial)heart.ProcessMaterial).EmissionBoxExtents = new Vector3(GetParent().GetViewport().Size.x*20, GetParent().GetViewport().Size.y*20, 0);
			GetParent().AddChild(heart);
			heart.OneShot = true;
			heart.Emitting = true;
			heart.Visible = true;
