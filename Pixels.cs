...
public class Pixels : Particles2D
{
	...
	public override void _Process(float delta)
	{
		if (Visible == true && Input.IsActionJustPressed("ESCAPE"))
		{
			...
			if (Visible == false && Input.IsActionJustReleased("PIXELS") && GetParent().GetChildCount() < 1000)
			{
				cooldown = 0;
			}
			if (Visible == false && cooldown <=0 && Input.IsActionPressed("PIXELS") && GetParent().GetChildCount() < 1000)
			{
