...
public class BarsRight : Particles2D
{
	...
	public override void _Ready()
	{
		...
		lifeTime = Lifetime;
		Vector2 ScreenLeft = new Vector2(GetParent().GetViewport().Size.x ,GetParent().GetViewport().Size.y / 2);
		Position = ScreenLeft;
	}
	...
	public override void _Process(float delta)
	{
		...
		if (Visible == false && Input.IsActionJustReleased("BARSRIGHT") && GetParent().GetChildCount() < 1000)
		{
			cooldown = 0;
		}
		if (Visible == false && cooldown<=0 && Input.IsActionPressed("BARSRIGHT") && GetParent().GetChildCount() < 1000)
		{
			Particles2D diamond = (Particles2D)Duplicate();
			Vector2 ScreenLeft =  new Vector2(GetParent().GetViewport().Size.x ,GetParent().GetViewport().Size.y / 2);
